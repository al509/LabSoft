...
#   TODO LIST:
    ...
    # 2. Лазер не должен выключаться с момента запуска программы,
    #    сделать таймер на 3 минуты.
    # 3. Работа оборудования должна идти отдельным потоком, так можно
    #    будет отслеживать процесс исполнения программы, а также останавливать
    #    весь процесс при необходимости.  Там же можно запусать импорт
    #    модуля apt.
    # 4. Построение графика N(x) во второй вкладке приложения.
    # 5. Сделать всю эту радость растягивающейся, или хотя бы подогнать
    #    под размер ноута.
